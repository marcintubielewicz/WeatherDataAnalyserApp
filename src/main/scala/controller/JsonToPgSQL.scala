 ...
 * and inserting the data into a PostgreSQL database.
 */
class JsonToPgSQL {

  // Processes JSON files by parsing, decoding, inserting data into a PostgreSQL database and deleting the processed files.
  ...
  /**
   * Inserts the weather data into the PostgreSQL database.
   * @param data means the weather data to be inserted.
   */
  ...
  /**
   * Reads weather data from the PostgreSQL database.
   * @return A sequence of weather data retrieved from the database.
   */
    ...
    try {
      ...
      while (resultSet.next()) {
        ...
        val weatherList = (3 to 6).map { index =>
          Weather(
            resultSet.getInt(s"weather_id$index"),
            resultSet.getString(s"weather_main$index"),
            resultSet.getString(s"weather_description$index"),
            resultSet.getString(s"weather_icon$index")
          )
        }
