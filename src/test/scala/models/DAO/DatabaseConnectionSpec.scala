  ...
  "DatabaseConnection" should "establish a connection to the database and retrieve data" in {
    ...
    try {
      ...
      while (resultSet.next()) {
        ...
        // Print the row data
        println(s"id: $id, coord_lon: $coordLon, coord_lat: $coordLat, weather_id: $weatherId, weather_main: $weatherMain, weather_description: $weatherDescription, weather_icon: $weatherIcon")
      }

      // Verify that at least one row is returned
      resultSet.next() shouldEqual true

      // Verify that the "id" column is not null
